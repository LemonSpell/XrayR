...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "${red}下载 XrayR $1 失败，请确保此版本存在（注意官方版本号通常带有v前缀，如 v0.9.4）${plain}"
            exit 1
        fi
    fi
