...
if [[ -f /etc/redhat-release ]]; then
    ...
    echo -e "${red}未检测到系统版本，请自行确认！${plain}\n" && exit 1
fi
...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        ...
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}检测 LemonSpell/XrayR 专属版本失败，可能是超出 Github API 限制，请稍后再试，或手动指定版本安装${plain}"
            exit 1
        fi
        echo -e "检测到 LemonSpell 专属库最新版本：${last_version}，开始安装"
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "${red}下载核心文件失败，请确保你的服务器能够下载 Github 的资产文件${plain}"
            exit 1
        fi
    else
        last_version=$1
        url="https://github.com/LemonSpell/XrayR/releases/download/v${last_version#v}/XrayR-linux-${arch}.zip"
        echo -e "开始安装 LemonSpell 专属库 XrayR $1"
        wget -q -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip ${url}
        if [[ $? -ne 0 ]]; then
            echo -e "${red}下载指定版本 $1 失败，请确保你的 Release 库中确实存在此版本包${plain}"
            exit 1
        fi
    fi
    ...
    echo -e "${green}LemonSpell 专属 XrayR ${last_version}${plain} 安装完成，已设置开机自启"
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        echo -e ""
        echo -e "全新安装，请先配置必要的内容 (vi /etc/XrayR/config.yml)"
    else
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/LemonSpell/XrayR/master/XrayR.sh
    ...
    echo -e ""
    echo -e "${green}LemonSpell XrayR使用方法: ${plain}"
    echo "------------------------------------------"
    echo "XrayR              - 显示管理菜单"
...
echo -e "${green}开始安装 LemonSpell XrayR${plain}"
install_base
install_acme
install_XrayR $1
